...
def capture_and_identify_faces():
    ...
    STRICT_THRESHOLD = 0.4  #  Exigência aumentada para evitar falsos positivos
    ...
    while True:
        ...
        for face_encoding, face_location in zip(face_encodings, face_locations):
            ...
            # Se não houver rostos conhecidos ou todas as distâncias forem altas, definir como "Desconhecido"
            if len(face_distances) == 0 or np.min(face_distances) > STRICT_THRESHOLD:
                name = "Desconhecido"
            else:
                # Pega os dois melhores índices para garantir que a diferença seja significativa
                sorted_indices = np.argsort(face_distances)
                best_match_index = sorted_indices[0]
                second_best_match_index = sorted_indices[1] if len(sorted_indices) > 1 else None

                # Verifica se a melhor correspondência é significativamente melhor que a segunda melhor
                if second_best_match_index is not None and (face_distances[best_match_index] + 0.1 > face_distances[second_best_match_index]):
                    name = "Desconhecido"  # Caso a diferença seja pequena, descarta o reconhecimento
                else:
